...
class MobileHomeOverviewTabRouterDelegate extends RouterDelegate<UriConfig>
    with ChangeNotifier, PopNavigatorRouterDelegateMixin<UriConfig> {
  MobileHomeOverviewTabRouterDelegate({
  ...
  AppNavigatorPage _createNavigatorPage(AppRoute route) {
    ...
    switch (route) {
      case MobileShoppingListOverviewRoute():
        // widget = MobileShoppingListOverviewScreen(
        //   key: Key(route.id),
        // );
        widget = Container(
          color: Colors.yellow,
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        ...
        final stackState = state.homeNavigationState!.tabStackStateMap[MobileHomeTab.overview]!;
        ...
        return Navigator(
          ...
          onDidRemovePage: (page) {
            ...
            _navigatorPresenter.onHomeTabRoutePopped(
              route: route,
              homeTab: MobileHomeTab.overview,
            );
          },
          observers: [
